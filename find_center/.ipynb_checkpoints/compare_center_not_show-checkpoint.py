...
def batch_vortex_identification (vx,vy,n,fig,ax,colorr):
    '''
...
def cmp_center(v_x,v_y,u_x,u_y,n):
    ...
    center[:3],strength[0] = batch_vortex_identification(df_vx,df_vy,n,fig, ax,'r')
    
    df_vx=pd.DataFrame(u_x)
    df_vy=pd.DataFrame(u_y)
    center[3:],strength[1] = batch_vortex_identification(df_vx,df_vy,n,fig, ax, 'b')
